...
function M.browse_file(opts)
	...
	local pickit = function(prompt_bufnr)
		...
		if content == nil then
			...
			elseif string.match(input, "^[A-z]:/?$") ~= nil then
				...
				curr_picker:refresh(
					new_finder(curr_picker.cwd, "*"),
					{ reset_prompt = true, new_prefix = build_prompt_prefix(curr_picker.cwd) }
				)
			...
			elseif string.match(input, "^[^/]+/.+") ~= nil then
				input = input:gsub("/", "*/") .. "*"
				curr_picker:refresh(
					new_finder(curr_picker.cwd, input),
					{ reset_prompt = true, new_prefix = build_prompt_prefix(curr_picker.cwd) }
				)
			elseif string.match(input, "/[^*]+*") ~= nil then
	...
	local function delete_file(prompt_bufnr)
		...
		if content ~= nil then
			vim.ui.input(
				{ prompt = "Delete file: ", default = curr_picker.cwd .. "/" .. content.value },
				function(input)
					if not input then
						return
					end
...
local function edit_qflist(type)
	...
	for _, data in ipairs(items) do
		loc_list[#loc_list + 1] =
			string.format("%s:%d:%d:%s", vim.fn.bufname(data["bufnr"]), data.lnum, data.col, data.text)
	end
